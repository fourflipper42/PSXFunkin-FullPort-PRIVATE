...
ENV_COUNT = 12
INTRO_FIRST = 0
ENV_FIRST = INTRO_FIRST + INTRO_COUNT
LIVE_FRAME_COUNT = ENV_FIRST + ENV_COUNT
FRAME_COUNT = 54  # Preserve the established stream size/CI contract; frames 36-53 are environment padding.

CHAR_IDLE_COUNT = 12
CHAR_LOCKED_COUNT = 6
CHAR_CONFIRM_COUNT = 8
CHAR_DENY_COUNT = 4
CHAR_IDLE_FIRST = 0
CHAR_LOCKED_FIRST = CHAR_IDLE_FIRST + CHAR_IDLE_COUNT
CHAR_CONFIRM_FIRST = CHAR_LOCKED_FIRST + CHAR_LOCKED_COUNT
CHAR_DENY_FIRST = CHAR_CONFIRM_FIRST + CHAR_CONFIRM_COUNT
CHAR_FRAME_COUNT = CHAR_DENY_FIRST + CHAR_DENY_COUNT
...
def quantize_frame(frame: Image.Image, *, dither: bool = True) -> tuple[list[int], list[tuple[int, int, int]]]:
...
def frame_record(frame: Image.Image) -> bytes:
    indices, colors = quantize_frame(frame, dither=True)
...
def char_frame_record(frame: Image.Image) -> bytes:
    # Characters get their own palette and nearly-native PS1 resolution.
    # Disabling dithering preserves Funkin's flat line/art colors instead of
    # turning them into the noisy 160x120 whole-scene approximation.
    indices, colors = quantize_frame(frame, dither=False)
    clut = [0] + [base.psx_color(c) for c in colors]
    pal = b"".join(struct.pack("<H", c) for c in clut)
...
def build_environment_scene(root: Path, anims: dict[str, AnimateAsset | None], i: int, count: int) -> Image.Image:
    """Build only the animated environment; characters are a separate bank."""
...
def build_character_overlay(root: Path, anims: dict[str, AnimateAsset | None], kind: str, i: int, count: int) -> Image.Image:
    """Render only BF/GF or locked character at 256x192 with transparency."""
    scene = Image.new("RGBA", (1280, 720), base.TRANSPARENT)
    ...
    # Same central 4:3 crop as the environment, but much higher resolution.
    return scene.crop((160, 0, 1120, 720)).resize((CHAR_W, CHAR_H), Image.Resampling.LANCZOS)


def build_lock_atlas(root: Path, menu_dir: Path, report: dict) -> None:
    """Use the actual v0.8.4 `charSelect/lock` Animate asset for grid locks."""
    asset = load_animate(root / "images" / "charSelect" / "lock")
    page = Image.new("RGBA", (256, 128), base.TRANSPARENT)
    labels = ("idle", "selected", "clicked")
    for index, label in enumerate(labels):
        frame_no = sample_frame(asset, 0, 1, label)
        image, _, _ = render_symbol(asset, frame_no)
        base.paste_box(page, image, (index * 64, 0, 64, 64))

    template = base.parse_tim_template(menu_dir / "story.tim")
    out = menu_dir / "cschar.tim"
    base.encode_tim4(page, template, out)
    report.setdefault("outputs", {})["cschar.tim"] = {
        "template": "story", "size": [256, 128], "bytes": out.stat().st_size,
        "sha256": sha256(out),
        "content": "official charSelect/lock Animate idle, selected and clicked frames",
    }
    for path in [asset.folder / "Animation.json", *sorted(asset.folder.glob("spritemap*.json")), *sorted(asset.folder.glob("spritemap*.png"))]:
        if path.is_file():
            report.setdefault("sources", {})[str(path.relative_to(root))] = {"sha256": sha256(path), "bytes": path.stat().st_size}
...
def write_first_frame_tim(path: Path, template: dict[str, int], frame: Image.Image, width: int, height: int, *, dither: bool = True) -> None:
    indices, colors = quantize_frame(frame, dither=dither)
    page = [0] * (template["width"] * template["height"])
    for y in range(height):
        page[y * template["width"]:y * template["width"] + width] = indices[y * width:(y + 1) * width]
...
def build_stayfunky(root: Path, psxavenc: Path, out: Path) -> Path:
    ...
    source = candidates[0]
    subprocess.run([str(psxavenc), "-t", "xa", "-f", "37800", "-b", "4", "-c", "2", "-F", "1", "-C", "0", str(source), str(out)], check=True)
...
def main() -> None:
    ...
    environment_frames = [build_environment_scene(root, anims, i, ENV_COUNT) for i in range(ENV_COUNT)]
    scene_frames = intro_frames + environment_frames
    # Keep the already-validated 54-record CSANIM layout for compatibility with
    # this checkpoint's build guards. The extra records are harmless repeats;
    # live runtime animation only consumes ENV_FIRST..ENV_FIRST+ENV_COUNT.
    while len(scene_frames) < FRAME_COUNT:
        scene_frames.append(environment_frames[(len(scene_frames) - LIVE_FRAME_COUNT) % ENV_COUNT])
    if len(scene_frames) != FRAME_COUNT:
        raise RuntimeError(f"built {len(scene_frames)} scene frames, expected {FRAME_COUNT}")

    character_frames = (
        [build_character_overlay(root, anims, "idle", i, CHAR_IDLE_COUNT) for i in range(CHAR_IDLE_COUNT)] +
        [build_character_overlay(root, anims, "locked", i, CHAR_LOCKED_COUNT) for i in range(CHAR_LOCKED_COUNT)] +
        [build_character_overlay(root, anims, "confirm", i, CHAR_CONFIRM_COUNT) for i in range(CHAR_CONFIRM_COUNT)] +
        [build_character_overlay(root, anims, "deny", i, CHAR_DENY_COUNT) for i in range(CHAR_DENY_COUNT)]
    )
    if len(character_frames) != CHAR_FRAME_COUNT:
        raise RuntimeError(f"built {len(character_frames)} character frames, expected {CHAR_FRAME_COUNT}")

    menu_dir = args.upstream / "iso" / "menu"
    menu_dir.mkdir(parents=True, exist_ok=True)
    back_template = base.parse_tim_template(menu_dir / "back.tim")
    title_template = base.parse_tim_template(menu_dir / "title.tim")
    if (back_template["width"], back_template["height"]) != (256, 256):
        raise SystemExit("unexpected back.tim template dimensions")
    if (title_template["width"], title_template["height"]) != (256, 256):
        raise SystemExit("unexpected title.tim template dimensions")

    stream = menu_dir / "csanim.bin"
    stream.write_bytes(b"".join(frame_record(frame) for frame in scene_frames))
    if stream.stat().st_size != FRAME_COUNT * RECORD_BYTES:
        raise RuntimeError("Character Select scene stream size mismatch")

    char_stream = menu_dir / "cschar.bin"
    char_stream.write_bytes(b"".join(char_frame_record(frame) for frame in character_frames))
    if char_stream.stat().st_size != CHAR_FRAME_COUNT * CHAR_RECORD_BYTES:
        raise RuntimeError("Character Select character stream size mismatch")

    csbg = menu_dir / "csbg.tim"
    write_first_frame_tim(csbg, back_template, environment_frames[0], SCENE_W, SCENE_H, dither=True)
    cslayer = menu_dir / "cslayer.tim"
    write_first_frame_tim(cslayer, title_template, character_frames[0], CHAR_W, CHAR_H, dither=False)
    ...
    build_lock_atlas(root, menu_dir, report)
    report["character_select_full"] = {
        ...
        "stream": {
            ...
            "ranges": {
                "intro": [INTRO_FIRST, INTRO_COUNT],
                "idle": [24, 12], "locked": [36, 6], "confirm": [42, 8], "deny": [50, 4],
                "environment": [ENV_FIRST, ENV_COUNT],
            },
            "compatibility_padding": "frames 36-53 repeat live environment frames; character states live in cschar.bin",
        },
        "character_stream": {
            "file": "cschar.bin", "sha256": sha256(char_stream), "bytes": char_stream.stat().st_size,
            "frame_size": [CHAR_W, CHAR_H], "frame_count": CHAR_FRAME_COUNT,
            "record_bytes": CHAR_RECORD_BYTES,
            "ranges": {
                "idle": [CHAR_IDLE_FIRST, CHAR_IDLE_COUNT],
                "locked": [CHAR_LOCKED_FIRST, CHAR_LOCKED_COUNT],
                "confirm": [CHAR_CONFIRM_FIRST, CHAR_CONFIRM_COUNT],
                "deny": [CHAR_DENY_FIRST, CHAR_DENY_COUNT],
            },
            "quality": "separate 256x192 4bpp character overlay with dedicated per-frame palette and no dithering",
        },
    ...
    report.setdefault("outputs", {})["csbg.tim"] = {
        ...
        "sha256": sha256(csbg), "content": "Character Select environment upload target; first live frame",
    }
    report.setdefault("outputs", {})["cslayer.tim"] = {
        "template": "title", "size": [256, 256], "bytes": cslayer.stat().st_size,
        "sha256": sha256(cslayer), "content": "high-fidelity 256x192 Character Select character overlay upload target",
    }
    args.report.write_text(json.dumps(report, indent=2) + "\n")

    print(f"Built Character Select environment: {FRAME_COUNT} frames x {SCENE_W}x{SCENE_H}, {stream.stat().st_size} bytes")
    print(f"Built Character Select characters: {CHAR_FRAME_COUNT} frames x {CHAR_W}x{CHAR_H}, {char_stream.stat().st_size} bytes")
    print(f"Ranges intro={INTRO_FIRST}+{INTRO_COUNT} env={ENV_FIRST}+{ENV_COUNT} char_idle={CHAR_IDLE_FIRST}+{CHAR_IDLE_COUNT} char_locked={CHAR_LOCKED_FIRST}+{CHAR_LOCKED_COUNT} char_confirm={CHAR_CONFIRM_FIRST}+{CHAR_CONFIRM_COUNT} char_deny={CHAR_DENY_FIRST}+{CHAR_DENY_COUNT}")
