...
#define MENU_CS_ENV_FIRST 24
#define MENU_CS_ENV_COUNT 12
#define MENU_CS_FRAME_COUNT 54

#define MENU_CS_CHAR_W 256
#define MENU_CS_CHAR_H 192
#define MENU_CS_CHAR_WORD_W (MENU_CS_CHAR_W / 4)
#define MENU_CS_CHAR_PIXEL_BYTES (MENU_CS_CHAR_W * MENU_CS_CHAR_H / 2)
#define MENU_CS_CHAR_RECORD_BYTES (MENU_CS_CLUT_BYTES + MENU_CS_CHAR_PIXEL_BYTES)
#define MENU_CS_CHAR_IDLE_FIRST 0
#define MENU_CS_CHAR_IDLE_COUNT 12
#define MENU_CS_CHAR_LOCKED_FIRST 12
#define MENU_CS_CHAR_LOCKED_COUNT 6
#define MENU_CS_CHAR_CONFIRM_FIRST 18
#define MENU_CS_CHAR_CONFIRM_COUNT 8
#define MENU_CS_CHAR_DENY_FIRST 26
#define MENU_CS_CHAR_DENY_COUNT 4
#define MENU_CS_CHAR_FRAME_COUNT 30
...
static void Menu_LoadCSFrames(void)
{
	...
	{
		sprintf(error_msg, "[Menu_LoadCSFrames] Character Select banks missing");
		ErrorLock();
	}
	menu_cs_uploaded_frame = 0xFF;
	menu_cs_uploaded_char_frame = 0xFF;
...
{
	// CHARSEL.XA is encoded with the same file number/channel expected by
	// PSXFunkin's XA filter (file 1, channel 0).
	Audio_StopXA();
...
{
	// Use the exact same top-left grid coordinates used to draw the icon/lock.
	// The previous build used a second coordinate system, causing the visible
	// selector to sit up-left from the item it was actually selecting.
	*x = FIXED_DEC(106 + ((menu_cs_x + 1) * 43), 1);
	*y = FIXED_DEC(62 + ((menu_cs_y + 1) * 45), 1);
}
...
static void Menu_CSDrawLock(u8 index, s32 x, s32 y)
{
	...
	// Real charSelect/lock Animate frames: idle, selected, clicked.
	u8 state = 0;
	if (index == menu_cs_grid)
		state = (menu_cs_mode == MenuCS_Deny) ? 2 : 1;
	RECT src = {state * 64, 0, 64, 64};
	RECT dst = {x, y, 32, 32};
	Gfx_DrawTexCol(&menu.tex_story, &src, &dst,
		tint[index][0] >> 1, tint[index][1] >> 1, tint[index][2] >> 1);
}
...
{
	RECT src = {92, 0, 32, 28};
	s32 x = (fx >> FIXED_SHIFT) - 6 - expand;
	s32 y = (fy >> FIXED_SHIFT) - 4 - expand;
	RECT dst = {x, y, 44 + (expand << 1), 40 + (expand << 1)};
	Gfx_DrawTexCol(&menu.tex_ng, &src, &dst, r, g, b);
}

static void Menu_CSDrawForeground(void)
{
	// Submit foreground before the character/environment layers: this OT is LIFO.
	if (menu_cs_mode == MenuCS_Confirm)
	{
		RECT src = {48, 68, 32, 28};
		RECT dst = {(menu_cs_cursor_x >> FIXED_SHIFT) - 7, (menu_cs_cursor_y >> FIXED_SHIFT) - 5, 46, 41};
		Gfx_DrawTex(&menu.tex_ng, &src, &dst);
	}
    ...
    character_case = r'''		case MenuPage_CharacterSelect:
		{
			if (menu.page_swap)
			{
				...
				if (menu_cs_seen_intro)
				{
					...
					Menu_SetCSFrame(MENU_CS_ENV_FIRST);
					Menu_SetCSCharFrame(MENU_CS_CHAR_IDLE_FIRST);
					Menu_StartCSMusic();
				}
				else
				{
			...
			else if (menu_cs_mode == MenuCS_Live)
			{
				...
				Menu_CSTickCursor();
				Menu_SetCSFrame(MENU_CS_ENV_FIRST + (u8)(((animf_count * 2) / 5) % MENU_CS_ENV_COUNT));
				if (menu_cs_grid == 4)
					Menu_SetCSCharFrame(MENU_CS_CHAR_IDLE_FIRST + (u8)(((animf_count * 2) / 5) % MENU_CS_CHAR_IDLE_COUNT));
				else
					Menu_SetCSCharFrame(MENU_CS_CHAR_LOCKED_FIRST + (u8)(((animf_count * 2) / 5) % MENU_CS_CHAR_LOCKED_COUNT));
			}
			else if (menu_cs_mode == MenuCS_Confirm)
			{
				Menu_CSTickCursor();
				Menu_SetCSFrame(MENU_CS_ENV_FIRST + (u8)(((animf_count * 2) / 5) % MENU_CS_ENV_COUNT));
				u8 frame = MENU_CS_CHAR_CONFIRM_FIRST + (u8)((menu_cs_timer * MENU_CS_CHAR_CONFIRM_COUNT) / 90);
				if (frame >= MENU_CS_CHAR_CONFIRM_FIRST + MENU_CS_CHAR_CONFIRM_COUNT)
					frame = MENU_CS_CHAR_CONFIRM_FIRST + MENU_CS_CHAR_CONFIRM_COUNT - 1;
				Menu_SetCSCharFrame(frame);
			...
			else
			{
				Menu_CSTickCursor();
				Menu_SetCSFrame(MENU_CS_ENV_FIRST + (u8)(((animf_count * 2) / 5) % MENU_CS_ENV_COUNT));
				u8 deny_frame = MENU_CS_CHAR_DENY_FIRST + (menu_cs_timer / 3);
				if (deny_frame >= MENU_CS_CHAR_DENY_FIRST + MENU_CS_CHAR_DENY_COUNT)
					deny_frame = MENU_CS_CHAR_DENY_FIRST + MENU_CS_CHAR_DENY_COUNT - 1;
				Menu_SetCSCharFrame(deny_frame);
			...
			// High-resolution character overlay is intentionally separate from the
...
'''
    ...
    replace_once(
        ...
        '\t\t\t\t<file name = "cschar.bin" type = "data" source = "iso/menu/cschar.bin"/>\n'
    ...
    text = menu.read_text()
    required = [
        "MENU_CS_FRAME_COUNT 54", "MENU_CS_CHAR_FRAME_COUNT 30", "CSANIM.BIN", "CSCHAR.BIN", "CSSFX.BIN", "CHARSEL.XA", "Menu_CSGridIndex",
        "Menu_CSDrawGrid", "Menu_SetCSCharFrame", "MenuCS_Intro", "MenuCS_Confirm", "MenuCS_Deny",
        "menu_cs_seen_intro", "MENU_CS_CHAR_CONFIRM_FIRST", "MENU_CS_CHAR_DENY_FIRST",
    ]
    ...
    xtext = xml.read_text()
    if xtext.count('name = "csanim.bin"') != 1 or xtext.count('name = "cschar.bin"') != 1 or xtext.count('name = "cssfx.bin"') != 1 or xtext.count('name = "charsel.xa"') != 1:
