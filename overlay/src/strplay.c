...
static int strNextVlc(STRENV *strEnv);
...
int PlayStr(int xres, int yres, int xpos, int ypos, STRFILE *str) {
	...
	*/
	...
	strPlayDone=0;
	strLastFrame=0;
	strFrameWidth = strFrameHeight = 0;
	strDoPlayback(str);
...
static void strDoPlayback(STRFILE *str) {
	...
	*/
	...
	if (!strNextVlc(&strEnv)) goto shutdown;

	while (1) {

		int last_frame = strLastFrame;
		int next_ok = 1;
		...
		// Get the next frame
		if (!last_frame) next_ok = strNextVlc(&strEnv);
...
static int strNextVlc(STRENV *strEnv) {
	...
	*/
	...
	// strNext already bounds its wait; avoid a nested WAIT_TIME-squared spin.
	if (next == 0) return 0;
...
static u_long *strNext(STRENV *strEnv) {
	...
	*/
	...
	// Buffers are sized for the declared viewport. Reject oversized or
	// changing dimensions before they can reach MDEC while another frame decodes.
	if (!sector->width || sector->width > strScreenWidth ||
	    !sector->height || sector->height > strScreenHeight ||
	    (strFrameWidth && (sector->width != strFrameWidth || sector->height != strFrameHeight))) {
		StFreeRing(addr);
		return 0;
	}
	strLastFrame = sector->frameCount >= strNumFrames;
