...
typedef enum {StageId_1_1, StageId_8_1, StageId_8_2} StageId;
...
typedef struct {StageId stage_id; StageDiff stage_diff; const StageDef *stage_def; CdlFILE music_file; u8 music_channel;} Stage;
...
int main(void) {
    StageDef def={7,6};Stage state={StageId_1_1,StageDiff_Erect,&def,{0,0},0};
    StageMusic_Load(&state);
    assert(state.music_channel==4 && state.music_file.size==800*2048 && state.music_file.pos==555);
    ...
    for(int d=StageDiff_Easy;d<=StageDiff_Hard;d++) {
        ...
        assert(state.music_channel==6 && state.music_file.pos==333 && state.music_file.size==20*2048);
