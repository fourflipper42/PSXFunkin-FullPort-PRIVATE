...
{
	IO_Data record = IO_Read("\\MENU\\CSUI8.BIN;1");
	if (record == NULL)
	{
		sprintf(error_msg, "[Menu_UploadCSHQUI] CSUI8.BIN missing");
		ErrorLock();
		return;
	}
	RECT clut = {MENU_CS_UI_CLUT_X, MENU_CS_UI_CLUT_Y, 256, 1};
	RECT image = {MENU_CS_UI_VRAM_X, MENU_CS_UI_VRAM_Y, MENU_CS_UI_WORD_W, MENU_CS_UI_H};
	LoadImage(&clut, (u32*)record);
	LoadImage(&image, (u32*)((u8*)record + 512));
	DrawSync(0);
	Mem_Free(record);
}
...
static void Menu_CSDrawUIRect(const RECT *src, const RECT *dst)
{
	...
	Gfx_Tex tex;
	tex.tim_mode = 1;
	tex.tpage = getTPage(1, 0, MENU_CS_UI_VRAM_X + page * 64, MENU_CS_UI_VRAM_Y);
	tex.clut = getClut(MENU_CS_UI_CLUT_X, MENU_CS_UI_CLUT_Y);
	tex.pxshift = 1;
	RECT local = {src->x - (page ? 128 : 0), src->y, src->w, src->h};
	Gfx_DrawTex(&tex, &local, dst);
}
    ...
    new_grid = r'''static void Menu_CSDrawGrid(void)
{
	...
	// Official v0.8.4 grid is 3x3. The original 107x127 spacing maps to
	// approximately 36x42 pixels in the PS1 center-cropped 320x240 view.
	for (u8 i = 0; i < 9; i++)
	{
		s32 x = 97 + (i % 3) * 36;
		s32 y = 40 + (i / 3) * 42;
	...
	// Nametag.hx centers the tag at (1008,100) in the 1280x720 view and scales
	// it to 0.77. The PS1 crop maps that to the top-right, around (283,33).
...
'''
    ...
    new_scene = '''\t\t\t\t// OT is LIFO. Submit foreground before characters before background\n\t\t\t\t// so the rendered order is background -> BF/GF -> foreground -> UI.\n\t\t\t\tMenu_CSDrawHQForeground(&scene_dst);\n\t\t\t\tMenu_CSDrawHQ8(&scene_dst);\n\t\t\t\tMenu_CSDrawHQ16(&scene_dst);'''
    text = once(text, old_scene, new_scene, 'v5 scene layer order')
    ...
    required = [
        ...
        '#define menu_cs_ui_vram_x 448',
        'menu_uploadcshqui', 'csui8.bin;1', 'csfg8.rle;1',
        'menu_setcsforegroundframe', 'menu_csdrawhqforeground',
        'official v0.8.4 grid is 3x3', 'rect name_dst = {219,17,128,32}',
    ]
    for marker in required:
        if marker not in low:
            raise SystemExit(f'quality v5 runtime missing {marker}')
    ...
    if 'stageid_8_' in low or 'spaghetti' in low:
        raise SystemExit('later milestone content leaked into Character Select v5')

    print('Applied Character Select quality v5: fixed font VRAM collision, split foreground, HQ official grid/selector/nametag')
