...
VRAM={
 ...
 'picobl':(448,0,0,480), 'darnbl':(512,0,16,480),
}
...
def compose_stage(data_root:Path, stage_name:str, images_subdir:str, out:Path, xref:float, yref:float, scale:float, vram_xs=(640,704), clut_xs=(48,64)):
    ...
    for p in props:
        ...
        if xml.exists() and anims: im=crop_sparrow(im,xml,anims[0].get('prefix'))
...
def main():
    ...
    custom=[('Shoot',lbl(p,'px','Shoot'),False,0),('ShootReturn',lbl(p,'px','Shoot and Return'),False,0),('GunReload',lbl(p,'px','Gun Reload'),False,0),('Hit',lbl(p,'px','Hit'),False,0),('Hey',lbl(p,'px','Hey'),False,0),('Cheer',lbl(p,'px','Cheer'),False,0)]
    ...
    a.report.parent.mkdir(parents=True,exist_ok=True); a.report.write_text(json.dumps({'characters':records},indent=2)+'\n'); print(json.dumps({'characters':records},indent=2))
