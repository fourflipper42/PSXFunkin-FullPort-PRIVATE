...
def describe(root, song, week, index):
    data = root / 'data/songs' / song
    meta = json.loads((data / f'{song}-metadata-erect.json').read_text())
    chart = json.loads((data / f'{song}-chart-erect.json').read_text())
    ...
    player = resolve_voices(folder, chars, 'player')
    opponent = resolve_voices(folder, chars, 'opponent')
...
def make_pair(ffmpeg, record, full, muted):
    ...
    for i, (_, offset) in enumerate(inputs):
        ...
        filters.append(f'[{i}:a]{timing}aresample=18900,aformat=channel_layouts=stereo[a{i}]')
...
def write_header(path, records):
    lines = ['/* Generated by build_remix_audio.py; do not edit. */',
             'static const StageMusicDef stage_music_defs[] = {']
    for r in records:
        ...
        lines.append(f'    {{StageId_{r["week"]}_{r["index"]}, {json.dumps(cd_path)}, '
                     f'{r["physical_sectors"]}u, {r["channel"]}, {{{speeds}}}}},')
    lines.append('};')
...
def verify_xa(path, ffprobe='ffprobe'):
    ...
    for i, block in enumerate(blocks):
        ...
        if (block[:4] != block[4:8] or block[0] != 1 or block[1] != i % 8
                or block[2] & 0x24 != 0x24 or block[3] != 0x05):
    ...
    expected = len(blocks) // 8
    if len(streams) != 8 or any(s.get('codec_name') != 'adpcm_xa'
            or int(s.get('sample_rate', 0)) != 18900 or s.get('channels') != 2
            or int(s.get('nb_read_frames', 0)) != expected for s in streams):
        raise ValueError(f'{path}: incomplete XA decode: {streams}')
    return dict(decoded_channels=8, decoded_sectors_per_channel=expected)
