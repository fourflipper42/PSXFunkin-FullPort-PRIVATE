...
def char_frame_record(frame: Image.Image) -> bytes:
    # Spatial CLUTs preserve BF, GF and foreground flat colours independently.
...
# The official ordering is environment -> GF/player -> speakers/foreground/UI.
# The v2 port incorrectly baked speakers and the choice-card foreground into the
# environment, so the later character overlay always covered them. Keep the
# back environment clean and composite those shipped foreground assets after
# BF/GF into the transparent high-resolution overlay.
...
# psxavenc emits consecutive 2336-byte XA audio sectors. PSXFunkin's existing
# 37.8kHz stereo XA assets use one matching audio sector every four physical
# sectors. Interleave three zero/nonmatching sectors so channel 0 is delivered
# at the intended 18.75 XA sectors/sec instead of roughly four times too fast.
old_xa = '''    subprocess.run([str(psxavenc), "-t", "xa", "-f", "37800", "-b", "4", "-c", "2", str(source), str(out)], check=True)\n    if out.stat().st_size == 0 or out.stat().st_size % 2336:\n        raise RuntimeError("CHARSEL.XA is not a valid raw XA sector stream")'''
new_xa = '''    subprocess.run([str(psxavenc), "-t", "xa", "-f", "37800", "-b", "4", "-c", "2", str(source), str(out)], check=True)\n    raw = out.read_bytes()\n    if not raw or len(raw) % 2336:\n        raise RuntimeError("CHARSEL.XA encoder output is not a valid 2336-byte XA sector stream")\n    zero = bytes(2336)\n    physical = bytearray()\n    for pos in range(0, len(raw), 2336):\n        physical.extend(raw[pos:pos + 2336])\n        physical.extend(zero)\n        physical.extend(zero)\n        physical.extend(zero)\n    out.write_bytes(physical)\n    if out.stat().st_size != len(raw) * 4:\n        raise RuntimeError("CHARSEL.XA physical interleave size mismatch")'''
...
new_env_upload = '''\tRECT clut_upload = {0, 510, 256, 1};'''
r = once(r, old_env_upload, new_env_upload, 'environment CLUT upload')
old_char_upload = '''\tRECT clut_upload = {\n\t\tmenu.tex_title.tim_crect.x,\n\t\tmenu.tex_title.tim_crect.y,\n\t\t16,\n\t\t1,\n\t};'''
new_char_upload = '''\tRECT clut_upload = {0, 511, 256, 1};'''
r = once(r, old_char_upload, new_char_upload, 'character CLUT upload')

# Use the lock art's own colours. The previous runtime applied a hard-coded
# green/cyan tint table on top of the already-quantized authentic lock frames.
...
{
	// Real charSelect/lock Animate frames, drawn with their authored palette.
	u8 state = 0;
...
# Draw the 4bpp frame as sixteen non-overlapping tiles, selecting the matching
# CLUT for each tile. This preserves the same pixel/tpage storage and avoids an
# 8bpp VRAM allocation while allowing up to 240 authored colours per layer.
...
new_draw = '''\t\t\t// This ordering table is LIFO: foreground UI was submitted above, then\n\t\t\t// characters/foreground art, then the back environment last. It renders\n\t\t\t// environment -> characters/speakers/card -> selector/locks/nametag.\n\t\t\tif (menu_cs_mode != MenuCS_Intro)\n\t\t\t\tMenu_CSDrawTiled(&menu.tex_title, &scene_dst, MENU_CS_CHAR_W, MENU_CS_CHAR_H, 511);\n\t\t\tMenu_CSDrawTiled(&menu.tex_back, &scene_dst, MENU_CS_FRAME_W, MENU_CS_FRAME_H, 510);'''
