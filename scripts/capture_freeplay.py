...
''')
    ...
    def commands(self,selection,time,confirm=-1):
        result=subprocess.run([str(self.exe),str(selection),str(time),str(confirm)],check=True,capture_output=True,text=True)
